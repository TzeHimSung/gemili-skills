...
把 24 个报告维度（21 个采集维度 + 3 个机构建模维度）所需的全部原始数据采集落地为 `.cache/{ticker}/raw_data.json`。

> **v3 registry 口径**：传统 `0_basic → 19_contests` 采集维度全部保留，`6_fund_holders` 与 `6_research` 分列后，采集维度共 21 个；Task 1 结束后，`run_real_test.py` 会自动触发 **Task 1.5 · 机构级建模**，向 raw_data.json 注入 3 个计算维度：
...
Agent 1 · 财报派 (Fundamentals)
  脚本: fetch_financials, fetch_fund_holders, fetch_valuation, fetch_research, fetch_moat
  维度: 1, 6_fund_holders, 6_research, 10, 14
  prompt: "你负责采集 {ticker} 的基本面数据。依次跑 5 个脚本，把输出汇总成一个 JSON 返回。脚本失败时降级到 web search 这只票的财报指标 ROE/净利率/PE/PB、公募基金持仓和券商研报观点。"
...
```
✅ Task 1/5 · 数据采集完成
[████████████████████] 100% · 21/24 维（21 采集维 + 3 机构建模维） · 用时 1m 23s
   ├─ Agent 1 (财报派)   ✓ 5/5 维
   ├─ Agent 2 (行情派)   ✓ 3/3 维
   ├─ Agent 3 (行业派)   ✓ 7/7 维
   └─ Agent 4 (情绪派)   ✓ 5/5 维
...
## 24 个报告维度（21 采集维 + 3 机构建模维） fetcher / compute 映射
...
| 6_fund_holders 公募基金持仓 | `fetch_fund_holders.py {ticker}` | `{name} 公募基金 持仓 基金经理` |
| 6_research 研报 | `fetch_research.py {ticker}` | `{name} 研报 评级 目标价` |
...
| 20 机构估值模型 | `compute_deep_methods.compute_dim_20` | 由已采集 raw_data 计算 |
| 21 研究工作流 | `compute_deep_methods.compute_dim_21` | 由 dim 20 + raw_data 计算 |
| 22 深度决策方法 | `compute_deep_methods.compute_dim_22` | 由 dim 20/21 + raw_data 计算 |

## 🌟 Bonus 数据 (v0.6+)

除了 24 个报告维度，还有 2 个额外 helper 必须在 Task 1 后期执行（先拉 basic → 再 spawn 以下 2 个并行任务）：
...
**执行时机**: 这 2 个放在 Task 1 最后一步（所有 fetcher 都完成后）跑，因为它们依赖已经写入 `raw_data.json` 的其他字段。
...
- [ ] 24 个 dimension key 全部存在（以 `FETCHER_REGISTRY` 为准）
